...
const add{{ table.name | friendly | capitalize }}Epic: Epic<{{ table.name | friendly | capitalize }}Action, {{ table.name | friendly | capitalize }}Action, IState> = (
  ...
  mergeMap((action) => {
    ...
    Object.keys(action.payload).map((item) => action.payload[item] && data.append(item, action.payload[item]._id ? action.payload[item]._id : typeof action.payload[item] === 'object' && action.payload[item].isPrototypeOf === 'File' ? JSON.stringify(action.payload[item]) : action.payload[item]))
...
const edit{{ table.name | friendly | capitalize }}Epic: Epic<{{ table.name | friendly | capitalize }}Action, {{ table.name | friendly | capitalize }}Action, IState> = (
  ...
  action$.pipe(
    ...
    mergeMap((action) => {
      ...
      Object.keys(action.payload).map((item) => action.payload[item] && data.append(item, action.payload[item]._id ? action.payload[item]._id : typeof action.payload[item] === 'object' ? JSON.stringify(action.payload[item]) : action.payload[item]))
